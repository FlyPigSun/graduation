...
class Personal_Activity_Model  extends  CI_Model{
    ...
    //找到所有教师推荐活动的id
    public function find_aid_teacherPush($sid,$date){
        $this->load->database();
        $sql="select * from personal_activity_tb where sid=? and is_push=1 and date=?";
    ...
    public function findInfo($aid,$sid){
        $this->load->database();
        $sql="select pa.sid,s.realname as name,s.studentnumber,a.* ,pa.is_finish,pa.end_time, pa.s_answer as answer from student_tb s
              inner join personal_activity_tb pa on s.id=pa.sid
    ...
    public function findSystemPush($sid){
        $this->load->database();
        $sql="select s.id,s.realname,s.studentnumber,s.gender,a.* ,pa.is_finish from student_tb s
              inner join personal_activity_tb pa on s.id=pa.sid
              inner join activity_tb a on pa.aid=a.id where pa.sid=? and is_push=2";
        $query=$this->db->query($sql,array($sid));
        $data=$query->result();
        $this->db->close();
        return $data;
